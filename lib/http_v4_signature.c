...
#define HMAC_SHA256(k, kl, d, dl, o)                                    \
  if(Curl_hmacit(Curl_HMAC_SHA256, k, kl, d, dl, o) != CURLE_OK) {      \
...
CURLcode Curl_output_v4_signature(struct connectdata *conn, bool proxy)
{
  ...
  const char *provider = data->set.str[STRING_V4_SIGNATURE] ?
    data->set.str[STRING_V4_SIGNATURE] : "aws:amz";
  ...
  *tmp = 0;
  ...
  HMAC_SHA256((unsigned char *)sk, strlen(sk), (unsigned char *)date,
              (unsigned int)strlen(date), tmp_sign0);
  sha256_to_hex(sha_hex, tmp_sign0, sizeof(sha_hex));

  HMAC_SHA256(tmp_sign0, sizeof(tmp_sign0), (void *)region,
              (unsigned int)strlen(region), tmp_sign1);
  HMAC_SHA256(tmp_sign1, sizeof(tmp_sign1), (void *)api_type,
              (unsigned int)strlen(api_type), tmp_sign0);
  HMAC_SHA256(tmp_sign0, sizeof(tmp_sign0), (void *)request_type,
